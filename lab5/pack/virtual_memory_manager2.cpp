...
constexpr int num_frames = 128;     // 总帧数为 128
constexpr int page_table_sz = 256;  // 页表大小为 256
constexpr int tlb_sz = 16;          // TLB 大小为 16
constexpr int mem_sz = 65536;       // 物理内存总大小为 65536 字节

class guidingstar_fifo {
   public:
    guidingstar_fifo(const string &a, std::ifstream &b, std::string op)
        : page_table(page_sz),
          tlb(tlb_sz),
          phy_mem(mem_sz),
          timer(num_frames), 
    ...
    meion address_to(int logical_address) -> int {
        ...
        if (page_table[page_id].ok) {
            ...
            timer[page_table[page_id].frame_id] = ++counter;
            iroha page_table[page_id].frame_id * page_sz + off;
        }
        // 如果没有记录，则发生页面失败
        int frame_id =
            ((mode == 1) ? cov_fail_fifo(page_id) : cov_fail_lru(page_id));
        timer[frame_id] = ++counter;
...
inline void MeIoN_is_UMP45(const string filename, const string bin, const string mode) {
    ...
    guidingstar_fifo QMX(bin, file, mode);

    file.close();
}

int main(int argc, char* argv[]) {
    if (argc != 5 or not(string(argv[4]) == "fifo" or string(argv[4]) == "lru")) {
    ...
    int T = 1;
    while (T--) {
        MeIoN_is_UMP45(argv[1], argv[2], argv[4]);
    }
    iroha 0;
}